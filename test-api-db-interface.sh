...
# ==========================================
# НАГРУЗОЧНОЕ ТЕСТИРОВАНИЕ KAFKA → API
# ==========================================
...
echo -e "${CYAN}════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}                        НАГРУЗОЧНОЕ ТЕСТИРОВАНИЕ KAFKA → API${NC}"
echo -e "${CYAN}════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════${NC}"
...
# Проверяем Target API интерфейс
TARGET_ID=$(curl -s "http://localhost:5003/api/Subscription/interfaces" | jq -r '.[] | select(.name=="LoadTest_API_Target") | .id // empty')
if [ -z "$TARGET_ID" ]; then
    echo -e "${YELLOW}   Создание Target API интерфейса...${NC}"
...
echo -e "${GREEN}   ✓ Target API интерфейс (ID: $TARGET_ID)${NC}"
...
CONNECTION_ID=$(curl -s http://localhost:5003/api/Subscription/connections | jq -r '.[] | select(.integrationPattern == "7" and .interfacePublicationId == '$SOURCE_ID') | .id')
...
# ------------------------------------------------------------------
# 3. ОЧИСТКА ЦЕЛЕВОЙ API
# ------------------------------------------------------------------
echo -e "\n${BLUE}🗑️ 3. ОЧИСТКА ЦЕЛЕВОЙ API${NC}"
...
docker logs integration-engine --tail 15 2>/dev/null | grep -E "KAFKA|WriteToApi|API" | tail -5

# ------------------------------------------------------------------
# ИТОГИ
# ------------------------------------------------------------------
echo -e "\n${CYAN}════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}                          📊 ИТОГИ ТЕСТИРОВАНИЯ KAFKA → API${NC}"
echo -e "${CYAN}════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════${NC}"
echo ""
echo -e "${GREEN}✅ Результаты:${NC}"
echo "   • Отправлено: ${MESSAGE_COUNT} сообщений"
echo "   • Пропускная способность: ${RPS} msg/s"
echo "   • Доставлено в API: ${RECEIVED_COUNT} сообщений"